...
<template>
  <div class="col py-3">
    ...
    <div v-else>
      <input type="text" class="form-control mb-2 input-lg" v-model="listDetail.name"
        @keyup.enter="editListTitle(false)">
    </div>

    <input type="text" class="form-control mb-2" v-model="newItemSubject" @keyup.enter="addToList"
      placeholder="hier neue Todo eintippen und Enter drucken">
    <ul class="list-group">
      <li class="list-group-item d-flex justify-content-between" v-for="(item, index) in uncheckedItems" :key="index">
    ...
    <div class="collapse" :class="showDone ? 'show' : ''">
      <ul class="list-group">
        <li class="list-group-item d-flex justify-content-between" v-for="(item, index) in checkedItems" :key="index">
          <ToDoListItem :item="item" @update="loadData" @open="openToDoDetail" />
        </li>
      </ul>
    </div>
